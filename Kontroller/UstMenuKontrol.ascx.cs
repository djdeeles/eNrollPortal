...
namespace EnrollPortal.Kontroller
{
    public partial class UstMenuKontrol : UserControl
    {
        ...
        public void AnaMenuler(Int32 UstMenuId, MenuItemCollection items)
        {
            ...
            var Menuler =
                Veriler.Menuler.Where(p => p.MenuLokasyonId == 1 && p.MenudeGoster && p.Durum && p.DilId == DilId).
            ...
            foreach (Menuler Menu in Menuler)
            {
                if (Menu.UstMenuId == UstMenuId)
                {
                    ...
                    else if (Menu.Url == "giris/cikis.aspx")
                    {
                        if (HttpContext.Current.User.Identity.IsAuthenticated)
                        {
                            MList.Add(oMenu);
                    ...
                    {
                        if (Menu.MenuAdi == "Kontrol Paneli")
                        {
                            if (HttpContext.Current.User.Identity.IsAuthenticated)
                            {
                                if (Enroll.YetkiAlaniKontrol(Convert.ToInt32(HttpContext.Current.User.Identity.Name)))
                                {
                                    MList.Add(oMenu);
                                }
                            }
